...
export default function LoginPageClient() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message =
        err instanceof ApiClientError ? err.message : 'Unable to sign in right now.'
      setError(message)
      toast.error(message)
    } finally {
      setIsLoading(false)
  ...
  return (
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 relative">
        <div className="absolute inset-0 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: "url('/home.jpeg')" }} />
        ...
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full px-12 text-center text-white">
          ...
          <h2 className="text-4xl font-serif font-bold mb-4">Find your perfect home with confidence</h2>
        ...
        <div className="w-full max-w-md mx-auto">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              ...
              <Input type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="h-11 bg-gray-50 border-gray-200 focus:bg-white" />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Password</label>
              <div className="relative">
                <Input type={showPassword ? 'text' : 'password'} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required className="h-11 pr-10 bg-gray-50 border-gray-200 focus:bg-white" />
